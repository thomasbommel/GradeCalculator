...
public class SchoolYearTable implements DataBaseTable<SchoolYear> {
    ...
    public void deleteFromDataBase(SQLiteDatabase db, SchoolYear year) {
        ...
        Log.d(this.getClass().toString(),"SchoolYear '"+year.getName()+"' created");
        MainActivity.getInstance().refreshMainActivity();
    }
    ...
    public SchoolYear getFromDataBase(SQLiteDatabase db, SchoolYear element) {
        Cursor findEntry = db.query(TABLE_NAME, null, COLUMN_NAME+"='"+element.getName()+"'", null, null, null, null);
        if(findEntry.getCount()==0){
            return null;
        }else if(findEntry.getCount()>1){
            Log.wtf(User.class.toString(),"there is more then 1 user with name:"+element.getName()+" for the current user.");
            throw new IllegalStateException("there is more then 1 user with name:"+element.getName()+" for the current user.");
        }
